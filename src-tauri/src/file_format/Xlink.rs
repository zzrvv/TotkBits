...
impl<'a> Xlink_rs<'a> {
    ...
    pub fn binary_to_yaml(&self, data: &[u8]) -> io::Result<String> {
        ...
        unsafe {
            let yaml_ptr = xlink_bindings::binary_to_yaml(rawdata.as_slice()).cast::<i8>();
            if yaml_ptr.is_null() {
            ...
            xlink_bindings::free_string(yaml_ptr as *mut i8);
            String::from_utf8(yaml_bytes).map_err(|error| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("XLink converter returned invalid UTF-8 YAML: {error}"),
                )
            })
        }
    }
